...
public class ConvenientBanner<T> extends LinearLayout {
     ...
     * @param args args[0]:left,args[1]:top,args[2]:right,args[3]:bottom
     * @return
     */
    public ConvenientBanner setPageIndicator(int[] page_indicatorId,int ...args) {
        ...
        for (int count = 0; count < mDatas.size(); count++) {
            ...
            //设置指示器的边距
            if (args.length == 4){
                int[] padding = new int[args.length];
                for (int i = 0; i < args.length; i++) {
                    padding[i] = args[i]>0?args[i]:0;
                }
                pointView.setPadding(padding[0],padding[1],padding[2],padding[3]);
            }else {
                //边距默认值
